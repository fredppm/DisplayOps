...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use gRPC to get host health status (which includes debug info)
    const healthResponse = await grpcService.getHealthStatus(hostId as string);
    
    // Convert gRPC response to debug status format
    const responseData = {
      success: true,
      debug_status: {
        host_status: healthResponse.host_status || {},
        display_statuses: healthResponse.display_statuses || [],
    ...
    console.error('Debug status gRPC error:', error);
